<!-- 
 ...
 -->
<template>
	<view class="dp-f ai-c jc-fe pt-10 mb-4">
		<view class="number-box dp-f ai-c ">
			<text class="fs-38 color-15  lh-38 fw-b">￥</text>
			<block v-for="(myIndex, index) in indexArr" :key="index">
				<swiper class="swiper" vertical="true" :current="myIndex" circular="true"
					v-bind:style="{color:color,width:myIndex == 10 ? '10px' : width+'px',height:height+'px',lineHeight:fontSize+'px',fontSize:fontSize+'px'}">
					<swiper-item>
...
<style lang="scss">
	...
	.swiper-item {
		// fs-28 color-15  lh-28 mt-8 mb-6 ta-r fw-b
		font-size: 38rpx;
		color: #151515;
		line-height: 28rpx;
		font-weight: 600;
